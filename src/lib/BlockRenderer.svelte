<script lang="ts">
            ...
            const observer = new IntersectionObserver(entries => {
                entries.forEach(e => {
                    if (e.isIntersecting) {
                        spotlightContent = {
                            type: block.type,
                            title: block.caption || block.title || 'Reference',
                            url:   block.url ?? null,
                            block,
                        };
                    }
    ...
    function checkCloze() {
        if (clozeAnswer.toLowerCase().trim() === block.answer?.toLowerCase().trim()) {
            clozeCorrect = true;
        }
    }
...
<div bind:this={blockElement} class="block-wrapper mb-6">
    ...
    {#if block.type === 'heading' || block.type === 'heading_2'}
        <h2 class="text-2xl font-black mt-10 mb-3 text-slate-800 tracking-tight border-b border-slate-100 pb-2">
            {block.content}
        </h2>
    ...
    {:else if block.type === 'text' || block.type === 'paragraph'}
        <p class="text-lg leading-relaxed text-slate-700 font-serif">
            {#each parseSegments(block.content) as seg}
                {#if seg.isLink}
                    <HoverCard title={seg.title ?? ''} conceptId={seg.id ?? ''}
                        on:spotlight={e => { spotlightContent = e.detail; }} />
                {:else}{seg.text}{/if}
            {/each}
        </p>
    ...
    {:else if block.type === 'math'}
        <div class="my-8 py-6 px-8 bg-slate-50 border border-slate-200 rounded-xl overflow-x-auto text-center">
            ...
            {#if block.caption}
                <p class="text-xs font-bold text-slate-400 mt-3 uppercase tracking-widest">{block.caption}</p>
            {/if}
        </div>
    ...
    {:else if block.type === 'image'}
        <figure class="my-8">
            ...
            {#if block.caption}
                <figcaption class="text-center text-sm font-bold text-slate-400 mt-3">{block.caption}</figcaption>
            {/if}
        </figure>
    ...
    {:else if block.type === 'cloze'}
        <div class="my-6 p-5 bg-blue-50 border border-blue-100 rounded-xl flex flex-wrap items-center gap-2 text-base font-serif">
            ...
            <input type="text" bind:value={clozeAnswer} on:keyup={checkCloze} disabled={clozeCorrect}
                class="w-28 sm:w-40 text-center border-b-2 bg-transparent focus:outline-none font-bold transition-all
                    {clozeCorrect ? 'text-emerald-600 border-emerald-500' : 'text-blue-900 border-blue-400 focus:border-blue-700'}"
                placeholder="...">
                        ...
                        <thead>
                            <tr class="bg-slate-800 text-white">
                                {#each block.headers as h}
                                    <th class="px-4 py-2.5 text-left font-black text-xs uppercase tracking-wider">{h}</th>
                                {/each}
                            </tr>
                        </thead>
                        <tbody>
                            {#each block.rows as row, ri}
                                <tr class="{ri % 2 === 0 ? 'bg-white' : 'bg-slate-50'} border-b border-slate-100">
                                    {#each row as cell}
                                        <td class="px-4 py-3 text-slate-700 font-serif">{cell}</td>
                                    {/each}
                                </tr>
                            {/each}
                        </tbody>
            ...
            <table class="w-full text-sm">
                {#if block.headers}
                    <thead class="bg-slate-100 border-b border-slate-200">
                        <tr>
                            {#each block.headers as h}
                                <th class="px-5 py-3 text-left font-black text-[11px] uppercase tracking-widest text-slate-600">{h}</th>
                            {/each}
                        </tr>
                    </thead>
                {/if}
                <tbody class="divide-y divide-slate-100">
                    {#each (block.rows ?? []) as row, ri}
                        <tr class="{ri % 2 === 0 ? 'bg-white' : 'bg-slate-50'} hover:bg-blue-50/40 transition-colors">
                            {#each row as cell, ci}
                                <td class="px-5 py-3 {ci === 0 ? 'font-bold text-slate-800' : 'text-slate-600 font-serif'}">{cell}</td>
                            {/each}
                        </tr>
                    {/each}
                </tbody>
    ...
    {:else if block.type === 'frq'}
        <div class="my-8 bg-white border-2 border-blue-100 rounded-2xl overflow-hidden shadow-sm">
            <div class="px-6 py-4 bg-blue-600 flex items-center justify-between">
                ...
                {#if block.points}
                    <span class="text-xs font-black bg-blue-500 text-white px-3 py-1 rounded-full shrink-0">{block.points} pts</span>
                {/if}
            </div>
            {#if block.context}
                <div class="px-6 py-4 bg-blue-50 border-b border-blue-100 font-serif text-slate-600 italic text-sm leading-relaxed">
                    {block.context}
                </div>
            {/if}
                ...
                {#each (block.parts ?? []) as part, i}
                    <div class="p-5">
                        <div class="flex items-start gap-3 mb-3">
                            <span class="w-6 h-6 rounded-lg bg-blue-100 text-blue-700 text-xs font-black flex items-center justify-center shrink-0 mt-0.5">
                        ...
                        {:else}
                            <div class="p-4 bg-emerald-50 border border-emerald-100 rounded-xl text-sm" in:fade>
                                ...
                                {#if part.scoring_guide}
                                    <p class="text-xs text-slate-400 mt-2 font-bold border-t border-emerald-100 pt-2">Scoring: {part.scoring_guide}</p>
                                {/if}
                            </div>
                        {/if}
                    </div>
        ...
        <div class="my-12 rounded-3xl overflow-hidden border border-slate-200 shadow-lg">
                    ...
                    <ul class="space-y-2.5">
                        {#each block.points as point}
                            <li class="flex items-start gap-3 text-slate-300 font-serif text-sm leading-relaxed">
                                <span class="text-blue-400 font-black shrink-0 mt-0.5">→</span>
                                {point}
                            </li>
                        {/each}
                    </ul>
            ...
            {#if block.concept_questions?.length}
                <div class="bg-white px-8 py-6">
                    <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-5">
                        Concept Questions — Connections, Cause &amp; Effect, Why/How
                    </p>
                    <div class="space-y-6">
                        {#each block.concept_questions as q, i}
                            <div class="border-l-4 border-blue-200 pl-5">
                                ...
                                <textarea bind:value={conceptAnswers[i]} placeholder="Think through it before revealing…" rows="2"
                                    class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-serif text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none mb-2">
                                </textarea>
                                {#if !conceptRevealed[i]}
                                    <button on:click={() => revealConcept(i)} class="text-xs font-bold text-blue-600 hover:underline">
                                        Reveal deep answer →
                                    </button>
                                {:else}
